import { Component } from '@angular/core';
import { NsService } from './ns.service';

// In Component selector , tempplateurl and styleurl are metadata which is configured in @Component tag
@Component({
  selector: 'app-root',
  template: `

    <p>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  Welcome to {{course}} {{4*4}} course </p>
    <!-- for loop -->
    <ul>
      <li *ngFor="let i of myArr">{{i}}</li>      
    </ul>
    
    <!-- if else condition -->
    <ul>
      <li *ngIf="myArr; else idTemp">typing Array is present</li>      
    </ul>

    <!-- interpolation -->
    <img src="{{image}}">Interpolation

    <!-- property binding -->
    <img [src]="image">PropertyBinding

    <!-- Event binding --> 
    <button (click)="goBack()">Go Back</button>

    <!-- else condition -->
    <ng-template #idTemp>Array is not present</ng-template>



    <!-- two way data binding -->
    <div style="padding-left: 20px; ">
      <h2>Two-way Binding Example</h2>    
        <input [(ngModel)]="fullName"/> <br/><br/>    
      <p> Hello {{fullName}} </p>  
    </div>


    <h1>{{message}}</h1>



<!-- *ngIf  if else condition-->
    <!-- <div *ngIf="courses.length >0; else itTemp" >
      List of Courses
    </div> -->

    <!-- <div *ngIf="courses.length==0">
      No courses yet
    </div> -->
    <!-- or -->

    <!-- <ng-template #itTemp>No courses yet</ng-template> -->

     <!-- Another way of if/else condtion-->
    <!-- <div [hidden]="courses.length==0">List of courses</div>
    <div [hidden]="courses.length>0">No courses yet</div> -->

    
    
    <!-- ng switch cases -->
    <ul class="nav nav-pills">
      <li [class.active] ="viewMode=='map'"><a (click)= "changeViewMode('map')">Map View</a></li>
      <li [class.active] ="viewMode=='list'"><a (click)= "changeViewMode('list')">List View</a></li>
    </ul>

    <div [ngSwitch]="viewMode">
      <div *ngSwitchCase="'map'">Map view Content</div>
      <div *ngSwitchCase="'list'">List view Content</div>
      <div *ngSwitchDefault>Nothing to show</div>
    </div>


    <!-- ng for -->
    <ul>
      <li *ngFor="let crs of coursesList; index as i">
        {{i}}- {{crs.name}}
      </li>
      <br>
<!-- 
      <li *ngFor="let crs of coursesList; even as isEven">
        {{crs.name}} <span *ngIf="isEven">(Even)</span>
      </li>
      <br>

      <li *ngFor="let crs of coursesList; odd as isodd">
        {{crs.name}} <span *ngIf="isodd">(Odd)</span>
      </li>
      <br>

      <li *ngFor="let crs of coursesList; first as f">
        {{crs.name}} <span *ngIf="f">(first)</span>
      </li> -->
    </ul>

    <!-- ngfor change detection -->
    <button (click)="onAdd()" >Add</button>
    <ul>
      <li *ngFor="let crs of coursesList">
        {{crs.name}}
        <button (click)="onRemove(crs)" >Remove</button>  
      </li>
      
    </ul>

    

  `,
  styleUrls: ['./app.component.css']
})



export class AppComponent {
  //courseslist for cheking for lopp
  coursesList= [
    {id:1, name:'course1'},
    {id:2, name:'course2'},
    {id:3, name:'course3'},
    {id:4, name:'course4'}
  ];

  onAdd(){
    this.coursesList.push({id:5, name:'course5'})
  }
  
  // onRemove(){
  //   this.coursesList.pop()
  // }
 
  onRemove(crs: any){
    let index = this.coursesList.indexOf(crs);
    if (index !== -1) {
      this.coursesList.splice(index, 1);
    }
  }


  // intialize viewMode intialyy as map
  viewMode= 'something else';

  //function to change mode
  changeViewMode(mode: string): void {
    this.viewMode = mode;
  }
  

  //courses array for cheking if confition
  courses =[];

  title = 'my-project';
  course = 'Angular';
  image = 'data:image/png;base64,'

  // creating object and asignng properties
  myObj ={
    color: 'red',
    material: 'soft' 
  };

  //defining array
  myArr = ['a','e','i','o'];

  //creating function
  goBack(): void{}

  fullName: string = ""; 


  // declaring service as a dependency
  //new parameter as per message parameter
  message: string ="";
  
  constructor(private nsService: NsService){ } 

  // it called after constructor has been called and variable initialized
  ngOnInit(){
    this.message = this.nsService.getMsg();  
  }





}
